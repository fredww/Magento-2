...
class General extends AbstractHelper
{
    ...
    const SUPPORTED_LOCAL = [ 'en_US', 'zh_CN' ];
    ...
    const WECHATPAY_STATUS_PENDING = 'payment/' . self::MODULE_CODE . '/pending_status';
    const WECHATPAY_STATUS_PROCESSING = 'payment/' . self::MODULE_CODE . '/processing_status';
    ...
    {
        $apiUserName = trim($this->getStoreConfig(self::WECHATPAY_USERNAME, $storeId));
    ...
    {
        $apiPassword = trim($this->getStoreConfig(self::WECHATPAY_PASSWORD, $storeId));
    ...
    {
        $apiToken = trim($this->getStoreConfig(self::WECHATPAY_TOKEN, $storeId));
        if(empty($apiToken)){
            $this->addTolog('error', 'YaBand WechatPay API Username not set');
        }
    ...
    {
        $orderId = $order->getId();
        return $this->generateOrderPayUrl($orderId, $this->getOrderAmountByOrder($order), $this->getPayCurrency($order->getStoreId()), $order->getIncrementId(), $this->getApiUserName(), $this->getApiPassword(), $this->getRedirectUrl(), $this->getNotifyUrl());
    }
    ...
    private function generateOrderPayUrl($orderId, $totalAmount, $currency, $description, $username, $password, $redirectUrl, $notifyUrl)
    {
